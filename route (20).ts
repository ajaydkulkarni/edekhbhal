...
export async function POST(req:Request){
  try{
    const data=schema.parse(await req.json()); const {user,membership}=await requireMembership(data.organizationId);
    ...
    const p=await prisma.$transaction(async tx=>{
      ...
      await audit({organizationId:data.organizationId,userId:user.id,action:"PROPERTY_CREATED",entityType:"Property",entityId:property.id,newValue:data as any},tx);
      return property;
    });
