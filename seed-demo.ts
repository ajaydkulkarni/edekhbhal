...
import { buildOffsets, zonedLocalToUtc } from "../src/lib/schedule";
...
// Makes local staging use easier without adding another dependency.
// Existing shell/Vercel environment variables always win over local files.
loadLocalEnvFile(".env.local");
loadLocalEnvFile(".env");

const prisma = new PrismaClient();

const CONFIRM = process.env.DEMO_SEED_CONFIRM;
const DEMO_ORG_ID = process.env.DEMO_ORG_ID ?? "seed_demo_org_edekhbhal";
const DEMO_ORG_NAME = process.env.DEMO_ORG_NAME ?? "eDekhbhal Demo Operations";
const DEMO_TIMEZONE = process.env.DEMO_TIMEZONE ?? "America/Denver";
const GENERATE_OCCURRENCES = (process.env.DEMO_SEED_GENERATE_OCCURRENCES ?? "true").toLowerCase() !== "false";
...
async function main() {
  console.log("Seeding eDekhbhal demo data...");
  console.log(`Target Organization: ${DEMO_ORG_NAME}`);
  console.log(`Timezone: ${DEMO_TIMEZONE}`);
  ...
  console.log("");
  console.log("Demo master data ready:");
  console.table(counts);
  console.log("");
  console.log("Demo login emails (use the existing staging magic-link flow):");
  for (const row of users) {
  ...
  if (GENERATE_OCCURRENCES) {
    console.log("");
    console.log("Generating/reconciling the next 48 hours of ScheduleOccurrences...");
    const { reconcileScheduleOccurrences } = await import("../src/lib/occurrenceGenerator");
    let totalCreated = 0;
    let totalSkipped = 0;
    ...
    console.log(`Generated ${totalCreated} occurrence(s); skipped ${totalSkipped} candidate(s) outside effective working hours.`);
