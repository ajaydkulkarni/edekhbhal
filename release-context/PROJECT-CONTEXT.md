...
**Current deployment status:** v0.7.0 Supervisor Dashboard / modern Web shell is deployed and field-validated; v0.8.0 Mobile UX / Localization package is prepared for Codespace compile validation before its additive staging migration and replacement Android APK build
...
Web-only Service Log is available to active ADMIN and PROPERTY_MANAGER users under Reports. One row represents one completed occurrence Task and uses occurrence/task snapshots plus server-authoritative execution timestamps. Columns include Property, Work Area, Task List / Schedule, sequence, Task performed, actual time, scheduled time, deviation, User, date, start and end time. Filters include date range, Property, Work Area, Schedule and User; columns are sortable and results are tenant-scoped.

---

## v0.7.0 — Supervisor Dashboard & Web UX Modernization

The Web Dashboard is an ADMIN / PROPERTY_MANAGER operational command center. USER-role members remain focused on mobile execution and are blocked from the management Dashboard.

Dashboard capabilities include:

- KPI cards for Users Online, Schedules In Progress, Tasks Completed Today, Attention Required and Average Deviation Today.
- Live Workforce with USER state, last login, active duration, Property, Work Area, current Task and time in Work Area.
- live Task-completion Activity Feed.
- latest 20 private evidence photos/videos using short-lived signed URLs.
- Attention Required for overdue, missed and partial ScheduleOccurrences.
- Today's Schedule Progress.
- approximately 12-second Web polling for the initial implementation.

### Presence architecture

The additive `user_presence` table was applied to staging. Mobile foreground heartbeat is best-effort approximately every 45 seconds. A recent heartbeat within roughly two minutes determines the Dashboard Online count. Heartbeats are operational telemetry and are intentionally not written to AuditLog on every beat.

The v0.7.0 Dashboard and modern role-aware navigation were field-validated successfully. Existing Properties, Work Areas, Tasks, Schedules, Reports, Team, Organization, Audit, Subscription and Logout screens were spot-checked after the visual/navigation modernization. Current Work Area/time in area, evidence carousel and mobile-to-Dashboard Activity Feed were also field-tested successfully.

The installed pre-v0.7 Android APK does not contain the heartbeat code, so workers using that old APK can appear as `Working · no heartbeat` and the Users Online KPI can remain zero. The next replacement APK must contain the heartbeat source.

---

## v0.8.0 — Mobile UX, Localization & Personal Reporting

This increment extends the USER mobile application while preserving the existing occurrence execution engine, camera-only evidence rules, server-authoritative timers, tenant isolation, append-only notes and v0.7 heartbeat architecture.

### Authentication and Profile

- Email remains the authentication identity.
- `User.name` is the editable Display Name; there is no separate username login identity.
- Mobile supports Email + Password after password setup while retaining email/magic-link authentication for initial access and password recovery.
- Passwords use one-way scrypt hashes and are never stored/logged as plaintext.
- `Session.authMethod` records PASSWORD or MAGIC_LINK to support safe recovery behavior.
- A normal password-authenticated session must provide the current password to change an existing password.
- A current magic-link-authenticated session may reset the password without the old password.
- Password changes revoke other sessions while retaining the current session.
- Mobile Profile adds Display Name, preferred language, password management and prominent confirmed Sign Out.

### Preferred language and translation

Initial supported languages are English, Hindi, Marathi, Gujarati, Bengali, Punjabi, Tamil, Telugu, Kannada, Malayalam, Spanish, French and Arabic. `User.preferredLanguage = null` means English/default.

Static worker-facing UI strings are bundled in the mobile application for all initial languages. Organization, Property and Work Area names remain operational identifiers and are not silently translated.

English ScheduleOccurrence / ScheduleOccurrenceTask snapshots remain authoritative. For a non-English preference, Schedule names, Task names and detailed Task instructions are translated server-side and cached in `ContentTranslation`. Cache identity includes Organization/source/field/language and a hash of the English source so changed source content does not reuse stale translations.

Dynamic translation provider order is:

1. `GOOGLE_TRANSLATE_API_KEY`, if configured; otherwise
2. `TRANSLATION_API_URL`, with optional `TRANSLATION_API_KEY`.

Translation provider failure or absence never blocks execution; the English source remains available. Translation secrets must be configured only in Vercel environment settings and never committed to GitHub.

### Text to Speech

Mobile uses Expo Speech / the device text-to-speech engine. A speaker control reads the current Task name and detailed instructions. Translated content uses the selected language locale when translation is available; otherwise the English source is spoken. Long instructions are chunked and spoken sequentially.

### Notes keyboard fix

Task and Schedule Notes use a top-positioned keyboard-aware modal. Android uses resize keyboard mode. The note text plus Save/Cancel actions must remain visible while typing.

### Personal Reports

The Report tab becomes searchable/paginated personal history with Today, 7 Days, 30 Days and Custom date ranges, plus search by Task, Schedule, Property or Work Area. Results are always Organization-scoped and USER-scoped and include status, planned duration, actual duration, deviation, Task details, evidence count and recorded Task/Schedule notes.
...
Migration: `supabase-v0.8.0-mobile-profile-i18n.sql`.

### Release gate

The v0.8.0 package must pass Web typecheck/build, Mobile clean install/typecheck and Expo Doctor before the database migration is applied. After staging API deployment and field regression, build one replacement Android APK that includes both the v0.7 foreground heartbeat and v0.8 mobile improvements.
