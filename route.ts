...
export async function POST(req:Request){
 try{
  ...
  const org=admin?.memberships[0];
  if(!admin||!org||!user)return NextResponse.json({error:"E2E identities are not ready."},{status:400});
  const adminUser=admin;
  const adminMembership=org;
  const assignedUser=user;

  const properties=await prisma.property.findMany({where:{organizationId:adminMembership.organizationId,name:{in:["E2E Property A","E2E Property B"]}}});
  const propertyA=properties.find(p=>p.name==="E2E Property A"),propertyB=properties.find(p=>p.name==="E2E Property B");
  if(!propertyA||!propertyB)return NextResponse.json({error:"Run /api/e2e/setup first."},{status:400});

  async function area(propertyId:string,name:string){
    const found=await prisma.workArea.findFirst({where:{propertyId,name}});
    return found?prisma.workArea.update({where:{id:found.id},data:{status:"ACTIVE"}}):prisma.workArea.create({data:{propertyId,name,status:"ACTIVE"}});
  }

  const workAreaA=await area(propertyA.id,"E2E Work Area A"),workAreaB=await area(propertyB.id,"E2E Work Area B");
  let task=await prisma.task.findFirst({where:{organizationId:adminMembership.organizationId,name:"E2E Reported Work Task"}});
  task=task?await prisma.task.update({where:{id:task.id},data:{status:"ACTIVE"}}):await prisma.task.create({data:{organizationId:adminMembership.organizationId,name:"E2E Reported Work Task",descriptionHtml:"<p>E2E deterministic Task.</p>",createdById:adminUser.id}});

  async function schedule(name:string,workAreaId:string){
    let s=await prisma.schedule.findFirst({where:{organizationId:adminMembership.organizationId,name}});
    if(!s)s=await prisma.schedule.create({data:{organizationId:adminMembership.organizationId,name,frequencyType:"ONE_TIME",startAt:new Date(Date.now()-300000),timezone:"America/Denver",workAreaId,createdById:adminUser.id}});
  ...
  async function occurrence(s:any,wa:any,property:any,assignedUserId:string|null,active:boolean){
    let o=await prisma.scheduleOccurrence.findFirst({where:{scheduleId:s.id}});
    const start=new Date(Date.now()-300000),end=new Date(Date.now()+1500000);
    ...
    if(!o)o=await prisma.scheduleOccurrence.create({data:{organizationId:adminMembership.organizationId,scheduleId:s.id,scheduledStartAt:start,scheduledEndAt:end,timezone:"America/Denver",...data}});
  ...
  const a=await occurrence(scheduleA,workAreaA,propertyA,assignedUser.id,true),b=await occurrence(scheduleB,workAreaB,propertyB,null,false);
  let bNote=await prisma.scheduleOccurrenceNote.findFirst({where:{occurrenceId:b.o.id,note:"[E2E] Property B visibility report"}});
  if(!bNote)bNote=await prisma.scheduleOccurrenceNote.create({data:{occurrenceId:b.o.id,scope:"SCHEDULE",note:"[E2E] Property B visibility report",createdById:adminUser.id}});

  const bReport=await prisma.reportedWorkItem.upsert({where:{sourceNoteId:bNote.id},update:{propertyId:propertyB.id,workAreaId:workAreaB.id,status:"NEW",noteText:bNote.note,sourceScheduleName:scheduleB.name,reportedById:adminUser.id,dismissedAt:null,dismissedById:null,linkedScheduleId:null,scheduleLinkedAt:null,scheduleLinkedById:null},create:{organizationId:adminMembership.organizationId,propertyId:propertyB.id,workAreaId:workAreaB.id,sourceNoteId:bNote.id,noteScope:"SCHEDULE",noteText:bNote.note,sourceScheduleName:scheduleB.name,reportedById:adminUser.id,reportedAt:bNote.createdAt}});
  ...
  return NextResponse.json({error:e instanceof Error?e.message:"Unable to prepare Reported Work fixture."},{status:400});
 }
}
