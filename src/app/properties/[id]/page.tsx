...
export default async function PropertyDetail({params}:{params:Promise<{id:string}>}){
  ...
  const property=await prisma.property.findUnique({where:{id},include:{organization:true,workAreas:{where:{status:"ACTIVE"},include:{qrCodes:{where:{status:"ACTIVE"},orderBy:{generatedAt:"desc"},take:1}},orderBy:{name:"asc"}}}});
  ...
  return <><Nav/><main className="container">
    ...
    <WorkAreaManager propertyId={property.id} workAreas={workAreas} canManage={canManage}/>
  </main></>;
}
