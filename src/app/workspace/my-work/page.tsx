...
import {claimOccurrence,completeOccurrenceTask,partiallyCompleteOccurrence,startOccurrence} from "@/lib/work-execution/actions";
import {listMyWork,listOccurrenceTasks,type OccurrenceTaskRow} from "@/lib/work-execution/server";
...
export default async function MyWorkPage({searchParams}:{searchParams:Promise<{message?:string;error?:string}>}){
 ...
 if(snapshot.role_code==="USER"){
  for(const item of work){
   if(item.status==="IN_PROGRESS")tasksByOccurrence[item.id]=await listOccurrenceTasks(context,item.id);
  }
 }
 ...
 return <main className="workspacePage">
  ...
  {snapshot.role_code!=="USER"?<section className="workspacePanel"><h2>USER execution role required</h2><p className="muted">ADMIN and SITE_MANAGER retain planning/management surfaces. My Work execution is intentionally a USER command boundary.</p></section>:
  <>
   <section className="workspacePanel"><span className="eyebrow">TASK EXECUTION FOUNDATION 01</span><h2>Claim → QR start → sequential Task execution → server completion</h2><p className="muted">Only the assigned Organization Membership can execute an IN_PROGRESS occurrence. Task state changes are explicit server commands; viewing Previous/Next content never changes state. Required evidence remains fail-closed until verified media metadata exists.</p></section>
   ...
   <section className="scheduleGrid">{work.map(item=>{
    ...
    return <article className="scheduleCard" key={item.id}>
     ...
     {item.status==="IN_PROGRESS"?<>
      ...
      <div className="scheduleTaskTimeline">{tasks.map(task=><div key={task.id}>
       <strong>{task.sequence}. {task.task_name_snapshot}</strong>
       <p>{instructionText(task.task_instructions_snapshot)}</p>
       <p className="muted">{task.status} · planned {task.planned_duration_minutes} min{task.actual_duration_seconds!==null?` · actual ${task.actual_duration_seconds}s`:""}{task.evidence_required?` · ${task.required_evidence_type} evidence required`:""}</p>
       {task.status==="IN_PROGRESS"&&task.evidence_required?<p><strong>Evidence gate:</strong> Completion stays blocked until verified {task.required_evidence_type?.toLowerCase()} evidence metadata is available. Production media upload/normalization is the next evidence-layer increment.</p>:null}
       {task.status==="IN_PROGRESS"&&!task.evidence_required?<form action={completeOccurrenceTask}>
      ...
      {hasCompleted&&hasRemaining?<form action={partiallyCompleteOccurrence}>
