...
export async function GET(req: Request) {
  ...
  if (!rec) {
    await audit({ action: "LOGIN_FAILED", result: "FAILURE", metadata: { reason: "invalid_or_expired_magic_link" } });
    return NextResponse.json({ error: "Invalid or expired authentication link" }, { status: 400 });
  }
  ...
  (await cookies()).set("edk_session", sessionToken, {
    httpOnly: true,
    sameSite: "lax",
