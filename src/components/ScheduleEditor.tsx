...
export function ScheduleEditor({canManage,workAreas,tasks,initial,defaults}:{canManage:boolean;workAreas:WorkAreaOption[];tasks:TaskOption[];initial?:InitialSchedule;defaults?:{workAreaId?:string;reportedWorkItemId?:string;suggestedName?:string}}){
 ...
 const [taskMode,setTaskMode]=useState<"LIBRARY"|"ADHOC">("LIBRARY"),[taskToAdd,setTaskToAdd]=useState("");
 const [adHocName,setAdHocName]=useState(""),[adHocDescription,setAdHocDescription]=useState(""),[adHocSave,setAdHocSave]=useState(false);
 const [error,setError]=useState(""),[saving,setSaving]=useState(false);
 const selectedWorkArea=workAreas.find(w=>w.id===workAreaId),activeTasks=tasks.filter(t=>t.status==="ACTIVE");
 const timeline=useMemo(()=>{let cursor=0;return items.map(item=>{const mins=durationMinutes(item.duration)??0,start=cursor;cursor+=mins;return{start,end:cursor,valid:mins>0}})},[items]);
 ...
 return <div className="scheduleEditor nextScheduleEditor">
  <section className="nextBuilderCard"><div className="builderStep"><span>1</span><div><strong>Where & document control</strong><small>Choose the location and, when applicable, identify the controlled checklist / SOP.</small></div></div>
   <div className="formGrid">
    ...
    <label>Work Area<select value={workAreaId} onChange={e=>setWorkAreaId(e.target.value)} disabled={!canManage}><option value="">Select Work Area</option>{workAreas.map(wa=>{const selectable=wa.status==="ACTIVE"&&wa.propertyStatus==="ACTIVE",retained=initial?.workAreaId===wa.id;if(!selectable&&!retained)return null;return <option key={wa.id} value={wa.id}>{wa.name} — {wa.propertyName}{selectable?"":" (Inactive)"}</option>})}</select><small className="muted">Property context is shown with every Work Area.</small></label>
    <label>Document / SOP Reference No.<input maxLength={150} value={documentReference} onChange={e=>setDocumentReference(e.target.value)} disabled={!canManage} placeholder="e.g. QMS-PRD-017"/><small className="muted">Optional controlled-document or checklist reference.</small></label>
    <label>Revision / Version<input maxLength={100} value={documentRevision} onChange={e=>setDocumentRevision(e.target.value)} disabled={!canManage} placeholder="e.g. Rev 03"/><small className="muted">Snapshotted into generated occurrences for historical auditability.</small></label>
   </div>
  </section>
  <section className="nextBuilderCard"><div className="builderStep"><span>2</span><div><strong>When</strong><small>Set a one-time date or a recurrence pattern.</small></div></div>
